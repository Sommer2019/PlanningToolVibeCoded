...
class TaskApiTest : AbstractIntegrationTest() {
    ...
    private fun createProject(user: String, name: String): String {
        val res = request("POST", "/api/planning/projects", """{"name":"$name"}""", user)
    ...
    fun `task lifecycle and per-user board`() {
        ...
        val created = request("POST", "/api/planning/projects/$pid/tasks", taskBody(), "TestUser1")
        assertEquals(201, created.statusCode())
        val taskId = field(created, "id")

        // Per-user board for TestUser1 shows the assigned task.
        val board = request("GET", "/api/planning/projects/$pid/board", user = "TestUser1")
        assertEquals(200, board.statusCode())
        assertTrue(board.body().contains(taskId))

        // Drag&drop status change.
        val patched = request("PATCH", "/api/planning/tasks/$taskId/status", """{"statusId":"$IN_PROGRESS_STATUS"}""", "TestUser1")
        assertEquals(200, patched.statusCode())

        val task = request("GET", "/api/planning/tasks/$taskId", user = "TestUser1")
    ...
    fun `required fields are validated server-side`() {
        ...
        val res = request("POST", "/api/planning/projects/$pid/tasks", taskBody(title = ""), "TestUser1")
    ...
    fun `non-member cannot access project tasks`() {
        ...
        val res = request("GET", "/api/planning/projects/$pid/tasks", user = "TestUser2")
    ...
    fun `locked task editable only by admin or creator`() {
        ...
        val add = request("POST", "/api/planning/projects/$pid/members", """{"userRef":"TestUser2"}""", "TestUser1")
        ...
        // Admin locks it.
        assertEquals(200, request("POST", "/api/planning/tasks/$taskId/lock", user = "TestAdmin").statusCode())

        // A member who is not the creator cannot edit.
        val forbidden = request("PUT", "/api/planning/tasks/$taskId", taskBody(title = "Hacked"), "TestUser2")
        assertEquals(403, forbidden.statusCode())

        // The creator still can.
        val ok = request("PUT", "/api/planning/tasks/$taskId", taskBody(title = "Edited by creator"), "TestUser1")
    ...
    fun `admin can add project status and tasks can use it`() {
        ...
        val statusRes = request("POST", "/api/planning/statuses", """{"name":"Review","projectId":"$pid"}""", "TestAdmin")
        ...
        val list = request("GET", "/api/planning/statuses?projectId=$pid", user = "TestUser1").body()
        assertTrue(
            listOf("Todo", "In Progress", "Done", "Review").all { list.contains("\"$it\"") },
            "status list missing entries: $list",
        )

        val taskRes = request("POST", "/api/planning/projects/$pid/tasks", taskBody(statusId = reviewId), "TestUser1")
        assertEquals(201, taskRes.statusCode())
    }

    @Test
    fun `non-admin cannot create global status`() {
        val res = request("POST", "/api/planning/statuses", """{"name":"Blocked"}""", "TestUser1")
        assertEquals(403, res.statusCode())
    }
}
