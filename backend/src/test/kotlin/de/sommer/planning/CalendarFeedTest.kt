...
class CalendarFeedTest : AbstractIntegrationTest() {
    ...
    fun `project iCal feed exposes tasks as VEVENTs`() {
        val pid = field(request("POST", "/cpp-api/planning/projects", """{"name":"Calendar"}""", "TestUser1"), "id")
        ...
        """.trimIndent()
        assertEquals(201, request("POST", "/cpp-api/planning/projects/$pid/tasks", taskBody, "TestUser1").statusCode())

        val tokenRes = request("POST", "/cpp-api/planning/calendar/feed-tokens", """{"projectId":"$pid"}""", "TestUser1")
